...
use App\View\Admin\CursoView; // New view for admin CRUD operations
use App\Controller\AuthController; // To check admin role
...
abstract class CursoController {
        ...
        if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST["nome"])) {
            ...
            try {
                $curso = Curso::criarCurso(
                    ...
                    $descricao,
                    (int) $duracao,
                    (float) str_replace(',', '.', $preco), // Handle comma as decimal separator
                    $status
                );
                $id = CursoDao::cadastrar($curso);
                self::$msg = "Curso cadastrado com sucesso!";
                // header("Location: ?p=admin_cursos_list"); // Redirect to list after successful cadastration
                // exit;
            } catch (\Exception $e) {
    ...
    public static function deletar(): void {
        ...
        header("Location: ?p=admin_cursos_list"); // Redirect if direct access without del param
        exit;
    }
    ...
    // Public methods for visitors/students
    public static function listarCursosPublico(): void {
        $cursos = CursoDao::listar();
        // Filter active courses if needed, or handle in view
        \App\View\CursosView::listar($cursos); // A new view for public course listing
    }
}