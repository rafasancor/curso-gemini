...
use App\Model\Aluno; // Assuming students are also users
use App\Model\Usuario; // For admin users
use App\Dal\AlunoDao;
use App\Dal\UsuarioDao;
use App\View\AuthView; // A new view for auth related pages
...
abstract class AuthController {
    ...
    public static function login(): void {
        if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST["email"])) {
            ...
            try {
                // Try to find in Alunos table (for regular users)
                ...
                if ($aluno && password_verify($senha, $aluno->getSenha())) {
                    ...
                    $_SESSION['user_role'] = 'aluno'; // Or 'user' if you prefer
                    header("Location: ?p=cursos"); // Redirect to a user-specific dashboard or courses page
                    exit;
                }

                // Try to find in Usuarios table (for admins)
                ...
                if ($usuario && password_verify($senha, $usuario->getSenha())) {
                    ...
                    if ($usuario->getRole() === 'admin') {
                        header("Location: ?p=admin_dashboard"); // Redirect to admin dashboard
                    } else {
                        header("Location: ?p=cursos"); // Regular user dashboard
                    }
                    exit;
                }
    ...
    public static function register(): void {
        if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST["nome"])) {
            ...
            try {
                ...
                // $_SESSION['user_id'] = AlunoDao::buscarPorEmail($email)?->getId();
    ...
    public static function recoverPassword(): void {
        if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST["cpf"])) {
            ...
            try {
                ...
                if ($aluno) {
                    // Simulate sending a temporary code/link
    ...
    public static function requireAdmin(): void {
        if (!self::isAdmin()) {
            header("Location: ?p=login"); // Or a 403 forbidden page
            exit;
        }
    }
