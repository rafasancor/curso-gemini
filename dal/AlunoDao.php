    ...
    public static function cadastrar(Aluno $aluno): int {
        try {
            ...
            $stmt->execute(array(
                ...
                $aluno->getCpf(),
                $aluno->getEmail(),
                password_hash($aluno->getSenha(), PASSWORD_DEFAULT), // Hash the password
                $aluno->getDataNascimento()
            ));
            return (int) $pdo->lastInsertId();
        } catch (\PDOException $e) {
            if ($e->getCode() === '23000') { // Duplicate entry for UNIQUE key (CPF or email)
        ...
        try {
            ...
            foreach ($resultados as $dados) {
                $alunos[] = Aluno::criarAluno(
                    ...
                    $dados["cpf"],
                    $dados["email"],
                    $dados["senha"], // Stored hashed password
