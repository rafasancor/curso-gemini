...
abstract class CursoDao {
    ...
    public static function listar(): array {
        try {
            $pdo = Conn::getConn();
            // Fetch total number of students enrolled in each course
            $stmt = $pdo->prepare("
            ...
            foreach ($resultados as $dados) {
                ...
                // Add total_alunos_matriculados to the course object dynamically if needed,
                // or handle it in the view. For now, we'll just pass the data as is.
    ...
    public static function editar(Curso $curso): void {
        try {
            ...
            if ($stmt->rowCount() === 0) { // No rows affected means no change or ID not found
