<?php
namespace App\View\Admin; // <-- Este é o namespace correto

use App\Model\Curso;

class CursoView { // <-- Este é o nome da classe correto
    ...
    public static function formulario(?string $msg = null, ?Curso $curso = null, string $csrfToken): void {
        ...
        <section>
            ...
            <form action="<?= isset($curso) ? "?p=admin_cursos_edit" : "?p=admin_cursos_add" ?>" method="post">
                ...
                <input type="text" id="preco" name="preco" value="<?= htmlspecialchars($curso?->getPreco() ?? "") ?>" placeholder="Ex: 99.90">
            ...
            <table>
                ...
                <?php else: ?>
                    <?php foreach($cursos as $dados):
                        $curso = $dados['curso_obj']; // Extract the Curso object
                    ?>
                        <tr>
