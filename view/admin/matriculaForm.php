...
use App\Model\Aluno; // For type hinting
use App\Model\Curso; // For type hinting
            ...
            <form action="<?= isset($matricula) ? "?p=admin_matriculas_edit" : "?p=admin_matriculas_add" ?>" method="post">
                ...
                <select id="id_aluno" name="id_aluno" required>
                    ...
                    <?php foreach($alunos as $aluno): ?>
                        <option value="<?= htmlspecialchars($aluno->getId()) ?>"
                            <?= ($matricula?->getIdAluno() ?? '') === $aluno->getId() ? 'selected' : '' ?>>
                ...
                <select id="id_curso" name="id_curso" required>
                    ...
                    ?>
                        <option value="<?= htmlspecialchars($curso->getId()) ?>"
                            <?= ($matricula?->getIdCurso() ?? '') === $curso->getId() ? 'selected' : '' ?>>
                ...
                <input type="datetime-local" id="data_matricula" name="data_matricula" value="<?= isset($matricula) ? date('Y-m-d\TH:i', strtotime($matricula->getDataMatricula())) : date('Y-m-d\TH:i') ?>" required>

                <label for="status">Status:</label>
                <select id="status" name="status">
                    <option value="ativa" <?= ($matricula?->getStatus() ?? '') === 'ativa' ? 'selected' : '' ?>>Ativa</option>
                    <option value="concluida" <?= ($matricula?->getStatus() ?? '') === 'concluida' ? 'selected' : '' ?>>Concluída</option>
                    <option value="cancelada" <?= ($matricula?->getStatus() ?? '') === 'cancelada' ? 'selected' : '' ?>>Cancelada</option>
                </select>
